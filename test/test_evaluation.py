...
class TrainingTestCase(unittest.TestCase):
    ...
    def run_basic_test(self, ndim, fn, *args):
        ...
        if ndim > 1:
            x = x.unsqueeze(0)
            y = y.unsqueeze(0)
        assert fn(x, y, *args) == (0.0, 0.0, 0.0)
        x = torch.tensor([1, 0, 1, 0, 1, 0, 1, 0])
        y = torch.tensor([1, 0, 0, 1, 1, 0, 1, 1])
        if ndim > 1:
            x = x.unsqueeze(0)
            y = y.unsqueeze(0)
        assert fn(x, y, *args) == (1.0, 1.0, 1.0)
    ...
    def test_fscore_precision_recall(self):
        ...
        tp, fp, fn = 0, 0, 0
        assert model_eval.fscore_precision_recall(tp, fp, fn) == (0.0, 0.0, 0.0)
